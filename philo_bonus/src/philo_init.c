...
void	init_philo(t_p_arg *p_arg_p, int counter)
{
	...
	p_arg_p->p.d_t_acc = init_sem_philo(counter_to_str, 1);
	if (p_arg_p->p.d_t_acc == SEM_FAILED)
		exit (0);
	sem_unlink(counter_to_str);
	p_arg_p->p.n_t_f_acc = init_sem_philo(counter_to_str, 1);
